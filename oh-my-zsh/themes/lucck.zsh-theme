...
prompt_context() {
  if [[ "$USERNAME" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    echo -n "${PR_RESET}${PR_RED}$USERNAME@${PR_YELLOW}%m${PR_RESET}:${PR_BRIGHT_YELLOW}%~%<<${PR_RESET}"
  else
...
${GREEN_START_P1}'
RPROMPT='${PR_RESET}$(git_prompt_info)$(svn_prompt_info)${PR_BLUE}%D{%R:%S %a %Y-%m-%d} ${GREEN_END}${PR_RESET}'
